from numpy import load, kaiser, fft, abs, arange, arcsin, pi, take, broadcast_to, matmul, linspace, \
                  sign, ones, absolute, meshgrid, exp, zeros,  mean
from numpy.linalg import eigh
from Libraries.utils.utils import askopenfile, root_dir
...
class Profiles:
    ...
    def __init__(self,
                 radar,                   # radar object
                 virtual_channels=arange(32),      # virtual channels of interest
                 min_range=0.5,
                 max_range=8,
        ...
        self.range_fft_freq = fft.rfftfreq(self.range_fft_order, self.ts)   # IF frequency
    ...
    def _update_dependent_variables(self):
        ...
        print('range scale', self.range_scale.shape)
        print('angular_scale', self.angular_scale.shape)
    ...
    def range(self, raw_data, calibrated=True):
        ...
        range_profile = fft.rfft(windowed_data,
    ...
    def music(self, range_profile, show_eigs=False, norm=False):
        """
